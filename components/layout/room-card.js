...
const RoomCard = ({ room, clicked, setShowModal, setShowRoom, bookmarkList }) => {
    ...
    return (
        <div className={`flex flex-col gap-y-2 sm:gap-x-2 sm:flex-row bg-white ${pathname === '/' ? 'w-[75vw]' : 'w-[92vw]'}  sm:w-[520px] h-[320px] sm:h-[174px] p-2 shadow-md rounded-md`}>
            {/**left */}
            <div className="w-[100%] sm:w-[260px] relative">
                {/* <img src={images[0]} alt="" className="rounded-md h-[200px] sm:h-[165px] w-[100%]" /> */}
                <Image src={images[0]} height={pathname === '/' ? (width <= 393 ? 285 : 240) : (width > 393 ? 210 : 220)} width={400} blurDataURL='' className="rounded-md"></Image>
                ...
                <div className='flex text-xs gap-x-1 text-gray-50 absolute left-1 bottom-2'>
                    <p className="bg-[#00000066] p-1 rounded-md">{roomCategory}</p>
                    <p className="bg-[#00000066] p-1 rounded-md">for {tenants} tenants</p>
                </div>
            </div>
            {/**Right */}
